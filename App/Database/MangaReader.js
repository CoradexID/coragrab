...
class Database {
  ...
  async getChapters(mangaId, limit = null) {
    let posts = '';
    if (limit == null) {
      posts = await this.query('SELECT * FROM wp_posts p JOIN wp_postmeta m ON p.ID = m.post_id WHERE meta_key = ? AND meta_value = ? DESC', ['ero_seri', mangaId]);
    } else {
      posts = await this.query('SELECT * FROM wp_posts p JOIN wp_postmeta m ON p.ID = m.post_id WHERE meta_key = ? AND meta_value = ? DESC LIMIT ?', ['ero_seri', mangaId, limit]);
    }
